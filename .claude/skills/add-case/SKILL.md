...
## 2. 記事内容からフェーズ（本番稼働／実証実験／不明）を判定する

記事本文を読み、実際に「本番稼働（実運用）」なのか「実証実験（パイロット・トライアル・PoC）」なのかを確認する。要約（summary）を書くために記事を読む工程と合わせて判定するとよい。
...
- `フェーズ不明`: 発表・契約締結・将来計画のみで稼働状況が本文に書かれていないなど、判定できない場合。無理に本番稼働/実証実験のどちらかに寄せない。

判定結果は次のステップで`phaseHints`に1つだけ入れる。

## 3. 候補データを `data/source-pool.json` に追記する
...
`phaseHints`にはステップ2で判定した値（`本番稼働フェーズ` / `実証実験フェーズ` / `フェーズ不明`）を1つだけ入れる（`vendorHints`等と同じく本文全体からの自動検出はしないため、省略すると自動的に`フェーズ不明`になってしまう）。
